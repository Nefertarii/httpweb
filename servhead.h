...
ssize_t Writefile(const char*filename, int socketfd, off_t begin, size_t &filecount);
...
int Accept() {
    ...
    while(1) {
        int connectfd = accept(fd, sa, salenptr);
        if (connectfd < 0) {
            if(errno == EINTR)
                continue;
...
int Sendfile(int sockfd, struct Cache cache) {  
    while(cache.remaining) {
